...
clean:
	...
	sudo rm -rf ./Munisso.PokeShakespeare.Web/bin
...
test-unit-frontend:
	docker run --rm --volume `pwd`:/tmp/app/ --volume `pwd`/tmp/cypress:/root/.cache/Cypress/ -w /tmp/app/ node:12.18.1-buster ./scripts/test_unit_frontend.sh
...
run-docker: build-docker
	docker run --rm --dns $(DNS) -p 5000:80 $(ENV_DUMMY) pokeshakespeare:latest
...
test-e2e-frontend: build-docker
	...
	docker run --rm --volume `pwd`:/tmp/app/ --volume `pwd`/tmp/cypress:/root/.cache/Cypress/ -w /tmp/app/ --network=$(E2E_NETWORK) cypress/base:12.18.0 ./scripts/test_e2e_frontend.sh
...
test-e2e-backend: build-docker
	...
	docker network create --driver bridge $(E2E_NETWORK)
	docker run --rm -d --dns $(DNS) -p 5000:80 --name $(E2E_HOST) $(ENV_DUMMY) --network=$(E2E_NETWORK) pokeshakespeare:latest
	docker run --rm --volume `pwd`:/tmp/app/ -w /tmp/app/ --env TEST_HOST=$(E2E_HOST) --network=$(E2E_NETWORK) mcr.microsoft.com/dotnet/core/sdk:3.1-alpine ./scripts/test_e2e_backend.sh
	docker stop $(E2E_HOST)
	docker network rm $(E2E_NETWORK)